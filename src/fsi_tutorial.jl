...
T_s = Triangulation(model_solid)
T_f = Triangulation(model_fluid)
...
T_Γout = BoundaryTriangulation(model,"outlet")
n_Γout = get_normal_vector(T_Γout)
...
T_Γfs = InterfaceTriangulation(model_fluid,model_solid)
n_Γfs = get_normal_vector(T_Γfs)

# Once we have all the triangulations, we can generate the quadrature rules to be applied each domain. This will be generated by calling the `LebesgueMeasure` function, that given a triangulation and an integration degree, it returns the Lebesgue integral measure $d\Omega$.
degree = 2*k
dΩₛ = LebesgueMeasure(T_s,degree)
dΩ_f = LebesgueMeasure(T_f,degree)

# Idem for the boundary part.
bdegree = 2*k
dΓₙ = LebesgueMeasure(T_Γout,bdegree)

# Idem for the interface part.
idegree = 2*k
dΓ_fs = LebesgueMeasure(T_Γfs,idegree)
...
# ```math
# a_s(\mathbf{u}^h_{\rm s},\mathbf{v}^h_{\rm s}) + a_f((\mathbf{u}^h_{\rm f},p^h_{\rm f}),(\mathbf{v}^h_{\rm f},q^h_{\rm f})) + a_{fs}((\mathbf{u}^h_{\rm s},\mathbf{u}^h_{\rm f}, p^h_{\rm f}),(\mathbf{v}^h_{\rm s},\mathbf{v}^h_{\rm f}, q^h_{\rm f})) = l_s(\mathbf{v}^h_{\rm s}) + l_f(\mathbf{v}^h_{\rm f}) + l_{f,\Gamma_N}(\mathbf{v}^h_{\rm f})\qquad\forall[\mathbf{v}^h_{\rm s},\mathbf{v}^h_{\rm f}, q^h_{\rm f}]^T\in Y,
# ```
# with the following definitions:

# - $a_s(\mathbf{u}^h_{\rm s},\mathbf{v}^h_{\rm s})$ is the bilinear form associated with the solid counterpart, defined as
# ```math
# a_s(\mathbf{u},\mathbf{v})=\int\varepsilon(\mathbf{v}):\boldsymbol{\sigma}_s(\epsilon(\mathbf{u}))\ d\Omega_{\rm S}.
# ```
a_s(u,v) = ∫( ε(v) ⊙ (σₛ ∘ ε(u)) )*dΩₛ

# - $a_f((\mathbf{u}^h_{\rm f},p^h_{\rm f}),(\mathbf{v}^h_{\rm f},q^h_{\rm f}))$ is the bilinear form associated with the fluid counterpart, defined as
# ```math
...
# +   (\mathbf{v}_{\rm f}-\mathbf{v}_{\rm S})̇\cdot(p_{\rm f}\mathbf{n}_{\rm fs})\\
# &-  \chi(\mathbf{n}_{\rm fs\cdot\boldsymbol{\sigma}^{dev}_f(\mathbf{v}_{\rm f})\cdot(\mathbf{u}^{\rm f}-\mathbf{u}_{\rm s})
# +  (q_{\rm f}\mathbf{n}_{\rm fs})\cdot(\mathbf{u}_{\rm f}-\mathbf{u}_{\rm s})\right) d\Gamma_{\rm FS}}\\
# \end{aligned}
# ```
...
const γ = 1.0
const χ = -1.0
a_fs((us,uf,p),(vs,vf,q)) = ∫( α*(vf.⁺-vs.⁻)⋅(uf.⁺-us.⁻) + (vf.⁺-vs.⁻)⋅(p.⁺*n_Γfs - n_Γfs⋅(σ_dev_f∘ε(uf.⁺))) + (q.⁺*n_Γfs-χ*n_Γfs⋅(σ_dev_f∘ε(vf.⁺)))⋅(uf.⁺-us.⁻) )*dΓ_fs
# OLD FUNCTION:
# function a_fs(x,y)
#   us_Γ, uf_Γ, p_Γ = x
#   vs_Γ, vf_Γ, q_Γ = y
...
# xe = get_cell_coordinates(T_Γfs)
# he = apply(x->norm(x[2]-x[1]),xe)
using Gridap.CellData
dim = num_cell_dims(T_Γfs)
h_Γfs = 1.0# ( get_contribution((∫(1)*dΓ_fs).^dim,T_Γfs) ) # Not tested this line yet
α = γ*μ_f/h_Γfs
...
# - $l_f(\mathbf{v}^h_{\rm f})$ is the linear form associated with the fluid counterpart, defined as
# ```math
# l_f(\mathbf{v},q)=\int\mathbf{v}\cdot f + qg\ d\Omega_{\rm F}.
# ```
l_f((v,q)) = ∫( v⋅f + q*g )*dΩ_f

# - $l_{f,\Gamma_N}(\mathbf{v}^h_{\rm f})$ is the linear form associated with the fluid Neumann boundary condition, defined as
# ```math
# l_{f,\Gamma_N}(\mathbf{v})=\int\mathbf{v}\cdot h\ d\Gamma_{\rm FS}.
# ```
l_f_Γn(v) = ∫( v⋅hN )*dΓₙ
...
writevtk(T,"trian", cellfields=["uhs" => uhs, "uhf" => uhf, "ph" => ph])
...
uhs_solid = restrict(uhs, T_solid)
uhf_fluid = restrict(uhf, T_fluid)
ph_fluid = restrict(ph, T_fluid)
writevtk(T_solid,"trian_solid",cellfields=["uhs"=>uhs_solid])
writevtk(T_fluid,"trian_fluid",cellfields=["ph"=>ph_fluid,"uhf"=>uhf_fluid])
...
T_ΓS = BoundaryTriangulation(model,["cylinder","interface"])
