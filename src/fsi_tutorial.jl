...
degree = 2*k
dΩₛ = LebesgueMeasure(Ω_s,degree)
dΩ_f = LebesgueMeasure(Ω_f,degree)

# Idem for the boundary part.
bdegree = 2*k
dΓ_out = LebesgueMeasure(Γ_out,bdegree)

# Idem for the interface part.
idegree = 2*k
dΓ_fs = LebesgueMeasure(Γ_fs,idegree)
...
dΓₛ = LebesgueMeasure(Γ_S,bdegree)
